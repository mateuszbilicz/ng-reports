...
const parseLogs = (logs: NgReportsLog[]) => {
  const translatedLogs = logs.map((log) => {
    switch (log.type) {
      ...
      case 'http':
        return `HTTP Request: ${JSON.stringify({ url: log.url, statusCode: log.statusCode, body: log.body, response: log.response })}`;
      case 'route':
        return `Route change: ${JSON.stringify({ path: log.path, queryParams: log.queryParams, fragment: log.fragment })}`;
      case 'click':
